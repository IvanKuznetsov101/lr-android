...
class LightRoomRepository @Inject constructor(
    ...
    suspend fun getLightRoomsInArea(box: BoundingBox): NetworkResult<List<LightRoomDTO>> {
        return safeApiCall {
            lightRoomService.getLightRoomsInArea(
                swLat = box.southWest.latitude,
    ...
    suspend fun createLightRoom(
        latitude: Double,
        longitude: Double,
        idEvent: Long
    ): NetworkResult<LightRoomDTO> {
        val lightRoomRequest =
            LightRoomRequest(latitude = latitude, longitude = longitude, eventId = idEvent)
    ...
    suspend fun getLightRoomByEventID(id: Long): NetworkResult<LightRoomDTO> {
